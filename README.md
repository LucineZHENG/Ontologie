...
-----------------------
Ce projet est une ontologie OWL/RDF dédiée aux nouilles asiatiques, couvrant
des spécialités représentatives de Chine, du Japon et de Corée. Elle modélise
de manière systématique les ingrédients, les techniques de fabrication, les
valeurs nutritionnelles, les méthodes de cuisson et les restrictions
alimentaires. Ce projet constitue le travail final du cours Web Sémantique
et Big-Data.
...
Structure des classes principales :
  ...
  Food (classe racine)
  ├── Nouilles
  │     Par façon de cuisine  : Soupe_de_nouilles / Nouilles_sautées /
  │                             Nouilles_sèches
  │     Par façon de fabr.    : Nouilles_faites_maison /
  │                             Nouilles_faites_à_la_machine
  ├── Accompagnements
  │     Fruits de mer / Légumes / Viande
  ├── Ingrédients
  │     Farine (Farine_gluten : Blé / Farine_sans_gluten : Sarrasin)
  │     Fécule (Patate_douce / Riz)
  ├── Restrictions_de_diète
  │     Aliments_sans_gluten / Aliments_végétariens
  └── Valeurs_nutritives
        Allergie / Nutritions
...
2. Charger l'ontologie et exécuter une requête SPARQL
   ...
   for row in g.query(query):
       name = row.nouille.split("#")[-1] if "#" in row.nouille \
              else row.nouille.split("/")[-1]
...
-------------------------------------------------
Dans le fichier RDF, certaines propriétés et instances utilisent « # »
comme séparateur final d'IRI, tandis que d'autres utilisent « / ».
Il est donc nécessaire de déclarer les deux préfixes dans chaque requête :

   PREFIX ns:  <...ontology-7#>
   PREFIX nou: <...ontology-7/>

L'utilisation d'un préfixe incorrect retourne silencieusement zéro résultat.
Vérifiez l'IRI exact directement dans Protégé ou dans le fichier .rdf
avant d'écrire vos requêtes.
...
--------------------------------------------
 ...
 4. Quels types de nouilles sont faciles à faire grossir (> 501 kcal) ?
 ...
 9. Nouilles instantanées vs nouilles au sarrasin : lesquelles sont
    plus caloriques ?
...
12. Pour les amateurs de repas lourds (salé, huilé ou pimenté),
    quelles nouilles leur plairaient ?
...
Le code complet et les résultats sont disponibles dans
Requetes_Sparql_en_python.ipynb et Exemple_d_utilisation.pdf.


Bibliographie
-------------
[1] Hou, G. G. (2010). Asian Noodles: Science, Technology, and Processing.
    John Wiley & Sons. https://doi.org/10.1002/9780470634370

[2] Staab, S., & Harth, A. (2004). Pizza Ontology.
    Stanford University. https://protege.stanford.edu/ontologies/pizza/pizza.owl

[3] FoodOntology. FoodOn: The Food Ontology.
    https://github.com/FoodOntology/foodon

[4] Shao, W. (2014). Chinese Dim Sum Culture.
    Southeast University Press. ISBN 9787564121280
